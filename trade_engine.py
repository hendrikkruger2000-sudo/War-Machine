...
class TradeEngine:
    ...
    def should_enter_trade(self):
        ...
        if i["ema8"] > i["ema21"] and i["rsi"] > 53:
            ...
            direction = "put"
        elif inversions.get("misalignment"):
        ...
        # ❌ Suppression logic
        if direction == "call" and i["rsi"] < 60 and i["momentum"] > 0.6:
            return None
        if direction == "put" and i["rsi"] > 40 and i["momentum"] > 0.6:
            return None
        if i["choppiness"] < 10 and i["momentum"] > 0.6:
            return None
        if 47 < i["rsi"] < 53 and i["momentum"] < 0.7:
            return None
        if m["volatility"] > 0.00035 and i["momentum"] < 0.7:
            return None

        # 📉 Candle direction confirmation
        c = self.cb.candle_data[-1]
        if direction == "call" and c["close"] <= c["open"]:
            return None
        if direction == "put" and c["close"] >= c["open"]:
            return None
        ...
        if i["rsi"] < 18 or i["rsi"] > 82: score += 1
        if m["volatility"] < 0.0004: score += 1
        if m["lower_proximity"] < 0.00015: score += 2
        if i["rsi"] > 55 and i["momentum"] < 0.1 and m["lower_proximity"] < 0.00015: score += 2

        # 🧬 Inversion flag weighting
        if inversions.get("compression_trigger"): score += 2
        if inversions.get("breakout_trigger"): score += 2
        if inversions.get("misalignment"): score += 1
        if inversions.get("volatility_surge"): score += 1
    ...
    def simulate_trade(self, direction, price, tick_time, duration=60, override=False, inversion=None):
    ...
    async def evaluate_trade(self, tick_time):
        # ⏳ Cooldown logic
        if self.last_trade_time and (tick_time - self.last_trade_time).total_seconds() < 60:
            return
    ...
    async def place_live_trade(self, direction, price, tick_time, duration=60):
        amount = self.trade_amount
        try:
            if direction == "call":
                trade_id, _ = await self.api.buy(self.symbol, amount, duration, check_win=False)
                print(f"[LIVE TRADE] CALL | ID:{trade_id} | Price:{price}")
            elif direction == "put":
                trade_id, _ = await self.api.sell(self.symbol, amount, duration, check_win=False)
                print(f"[LIVE TRADE] PUT | ID:{trade_id} | Price:{price}")
            else:
                return

            # ✅ Move this BEFORE result check
    ...
    async def check_live_trade_result(self, trade_id):
        try:
